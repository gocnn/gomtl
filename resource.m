...
void *Device_NewBufferWithLength(void *device, size_t length,
                                 uint16_t options) {
  return
...
void *Device_NewBufferWithBytes(void *device, const void *bytes, size_t length,
                                uint16_t options) {
...
void *Buffer_Contents(void *buffer) { return ((id<MTLBuffer>)buffer).contents; }

void *Device_NewTextureWithDescriptor(void *device,
...
void Texture_ReplaceRegion(void *texture, struct Region region, uint_t level,
                           void *pixelBytes, size_t bytesPerRow) {
